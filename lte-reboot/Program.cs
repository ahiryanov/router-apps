...
class Program
{
	...
	{
		string nolteModemlog = $"{_logFile}-{device.Name}-nolte";
		...
		if (currentCount > _restartCount) //no lte reboot modem every {restartCount} times of reconnect reason NoLTE
		{
			File.WriteAllText(nolteModemlog, "1");
